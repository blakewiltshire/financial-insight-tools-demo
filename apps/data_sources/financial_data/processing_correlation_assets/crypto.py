...
# Cryptocurrency
# -------------------------------------------------------------------------------------------------
def load_and_clean_cryptocurrency():
    """
    Load and clean assets under 'Cryptocurrency', choosing either default or
    ...
    if source_type == "Preloaded Asset Types (User)":
        asset_category = "Cryptocurrency (User)"
        ...
        for cleaned_name, _ in user_assets.items():
            ...
            if asset_path:
                try:
                    ...
                    data_frame, "cryptocurrencies_user", cleaned_name)
        ...
        for asset_name, asset_file in asset_files.get("Cryptocurrency", {}).items():
            try:
                data_frame = pd.read_csv(asset_file)
                cleaned_data = clean_generic_data(data_frame, "cryptocurrencies", asset_name)
                cleaned_data_frames.append(cleaned_data)
            except (FileNotFoundError, ParserError, OSError, ValueError) as error:
