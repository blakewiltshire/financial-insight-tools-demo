...
# Market Indices
# -------------------------------------------------------------------------------------------------
def load_and_clean_market_indices():
    """
    Load and clean assets under 'Market Indices', choosing either default or
    ...
    if source_type == "Preloaded Asset Types (User)":
        asset_category = "Market Indices (User)"
        ...
        for cleaned_name, _ in user_assets.items():
            ...
            if asset_path:
                try:
                    ...
                    data_frame, "market_indices_user", cleaned_name)
        ...
        for asset_name, asset_file in asset_files.get("Market Indices", {}).items():
            try:
                data_frame = pd.read_csv(asset_file)
                cleaned_data = clean_generic_data(data_frame, "market_indices", asset_name)
                cleaned_data_frames.append(cleaned_data)
            except (FileNotFoundError, ParserError, OSError, ValueError) as error:
