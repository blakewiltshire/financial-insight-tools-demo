...
# -------------------------------------------------------------------------------------------------
from ..processing_correlation import clean_generic_data
from ..asset_map import asset_files
from ..user_asset_map import get_user_asset_path
from ..user_preloaded_assets import get_user_preloaded_assets

# -------------------------------------------------------------------------------------------------
# Equities - Magnificent Seven
# -------------------------------------------------------------------------------------------------
def load_and_clean_equities_7():
    """
    Load and clean assets under 'Equities - Magnificent Seven', choosing either default or
    ...
    if source_type == "Preloaded Asset Types (User)":
        asset_category = "Equities - Magnificent Seven (User)"
        ...
        for cleaned_name, _ in user_assets.items():
            ...
            if asset_path:
                try:
                    ...
                    data_frame, "equities_mag7_user", cleaned_name
                    )
        ...
        for asset_name, asset_file in asset_files.get("Equities - Magnificent Seven", {}).items():
            try:
                data_frame = pd.read_csv(asset_file)
                cleaned_data = clean_generic_data(data_frame, "equities_mag7", asset_name)
                cleaned_data_frames.append(cleaned_data)
            except (FileNotFoundError, ParserError, OSError, ValueError) as error:
...
# -------------------------------------------------------------------------------------------------
def load_and_clean_equities_const():
    """
    Load and clean assets under 'Equities - Sector Constituents', choosing either default or
    user-defined sources based on session selection. Consolidates all cleaned DataFrames for:
    - Correlation analysis
    - Volatility classification
    - Multi-period return calculation
    ...
    if source_type == "Preloaded Asset Types (User)":
        asset_category = "Equities - Sector Constituents (User)"
        user_assets = get_user_preloaded_assets().get(asset_category, {})
        ...
        for cleaned_name, _ in user_assets.items():
            ...
            if asset_path:
                try:
                    data_frame = pd.read_csv(asset_path)
                    cleaned_data = clean_generic_data(
                    data_frame, "equities_constituents_user", cleaned_name)
                    cleaned_data_frames.append(cleaned_data)
                except (FileNotFoundError, ParserError, OSError, ValueError) as error:
                    st.warning(f"Skipping user asset '{cleaned_name}': {error}")
    else:
        for asset_name, asset_file in asset_files.get("Equities - Sector Constituents", {}).items():
            try:
                data_frame = pd.read_csv(asset_file)
                cleaned_data = clean_generic_data(data_frame, "equities_constituents", asset_name)
                cleaned_data_frames.append(cleaned_data)
            except (FileNotFoundError, ParserError, OSError, ValueError) as error:
                st.warning(f"Skipping default asset '{asset_name}': {error}")
