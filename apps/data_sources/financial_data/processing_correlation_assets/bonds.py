...
# Short-Term Bonds
# -------------------------------------------------------------------------------------------------
def load_and_clean_short_term_bonds():
    """
    Load and clean assets under 'Short-Term Bonds', choosing either default or
    ...
    if source_type == "Preloaded Asset Types (User)":
        asset_category = "Short-Term Bonds (User)"
        ...
        for cleaned_name, _ in user_assets.items():
            ...
            if asset_path:
                try:
                    ...
                    data_frame, "short_term_bonds_user", cleaned_name)
        ...
        for asset_name, asset_file in asset_files.get("Short-Term Bonds", {}).items():
            try:
                data_frame = pd.read_csv(asset_file)
                cleaned_data = clean_generic_data(data_frame, "short_term_bonds", asset_name)
                cleaned_data_frames.append(cleaned_data)
            except (FileNotFoundError, ParserError, OSError, ValueError) as error:
...
# Long-Term Bonds
# -------------------------------------------------------------------------------------------------
def load_and_clean_long_term_bonds():
    """
    Load and clean assets under 'Long-Term Bonds', choosing either default or
    user-defined sources based on session selection. Consolidates all cleaned DataFrames for:
    - Correlation analysis
    - Volatility classification
    - Multi-period return calculation
    ...
    if source_type == "Preloaded Asset Types (User)":
        asset_category = "Long-Term Bonds (User)"
        user_assets = get_user_preloaded_assets().get(asset_category, {})
        ...
        for cleaned_name, _ in user_assets.items():
            ...
            if asset_path:
                try:
                    data_frame = pd.read_csv(asset_path)
                    cleaned_data = clean_generic_data(
                    data_frame, "long_term_bonds_user", cleaned_name)
                    cleaned_data_frames.append(cleaned_data)
                except (FileNotFoundError, ParserError, OSError, ValueError) as error:
                    st.warning(f"Skipping user asset '{cleaned_name}': {error}")
    else:
        for asset_name, asset_file in asset_files.get("Long-Term Bonds", {}).items():
            try:
                data_frame = pd.read_csv(asset_file)
                cleaned_data = clean_generic_data(data_frame, "long_term_bonds", asset_name)
                cleaned_data_frames.append(cleaned_data)
            except (FileNotFoundError, ParserError, OSError, ValueError) as error:
                st.warning(f"Skipping default asset '{asset_name}': {error}")
